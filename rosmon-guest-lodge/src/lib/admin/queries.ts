...
/**
 * Provisional capacity check used by the admin UI only.
 * The final availability algorithm (unit assignment + hold engine) lands in a
 * later milestone; this counts occupying bookings and blocks against the
 * physical unit count so staff are never shown a blind "always available".
 */
export function provisionalAvailability(opts: {
  ...
  const blocked = blocks.filter((bl) => {
    if (!overlaps(checkIn, checkOut, bl.start_date, bl.end_date)) return false;
    if (bl.room_type_id === roomType.id && !bl.room_unit_id) return true;
    const unit = activeUnits.find((u) => u.id === bl.room_unit_id);
    return Boolean(unit);
  }).length;

  const remaining = capacity - occupied - blocked;
  return { capacity, occupied, blocked, remaining, available: remaining > 0 };
}
