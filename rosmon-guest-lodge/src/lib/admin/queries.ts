...
/* ------------------------------------------------------------------ */
/* Unit status derivation — the ONE inventory engine                   */
/* ------------------------------------------------------------------ */
 ...
 * inventory blocks. This is authoritative, not provisional — it is the
 * same predicate public.create_booking() and public.room_type_availability()
 * enforce server-side (see the 20260901 migration). Every admin screen
 * that needs a unit's status (dashboard, rooms & inventory, calendar)
 * must call this — or computeUnitStatuses()/summarizeUnitStatuses() below
 * — rather than re-deriving it. Three screens once each had their own
 * version of this check and disagreed with each other; that is the bug
 * this function (and the two below) exist to close off for good.
 */
...
/** unitStatusOn() applied to every active unit for one date. Use this (not a hand-rolled loop) anywhere a full-inventory snapshot is needed. */
export function computeUnitStatuses(opts: {
  units: RoomUnitRow[];
  bookings: BookingRow[];
  blocks: InventoryBlockRow[];
  date: string;
  ...
  return opts.units
    ...
    .map((unit) => ({
      unit,
      ...unitStatusOn({ unit, bookings: opts.bookings, blocks: opts.blocks, date: opts.date }),
    }));
}

/** Counts of each status across a set of unit-status rows. Always sums to rows.length. */
export function summarizeUnitStatuses(
  rows: UnitStatusRow[],
): Record<UnitStatus, number> {
  return unitStatuses.reduce(
    (acc, s) => {
