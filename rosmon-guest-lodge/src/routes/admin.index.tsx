...
function Dashboard() {
  ...
  // Same engine as Rooms & Inventory and the calendar — see
  // lib/admin/queries.ts. "Occupancy" here means booked + held + occupied
  // (any unit with a booking attached), out of every active unit; blocked
  // and maintenance units are shown separately rather than folded in.
  ...
  const counts = summarizeUnitStatuses(statuses);
  const capacity = statuses.length || 15;
  const withBooking = counts.booked + counts.held + counts.occupied;
  const occupancy = Math.round((withBooking / capacity) * 100);

  const upcoming = rows
    .filter((b) => b.check_in >= todayStr && !["cancelled", "expired"].includes(b.booking_status))
    .slice(0, 6);

  const loading = bookings.isLoading || units.isLoading || blocks.isLoading;
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Stat
          ...
          hint={
            loading
              ? undefined
              : `${withBooking} of ${capacity} booked` +
                (counts.maintenance + counts.blocked > 0
                  ? ` · ${counts.maintenance + counts.blocked} unavailable`
                  : "")
          }
        />
        <Stat
