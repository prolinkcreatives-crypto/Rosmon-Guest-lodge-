...
function CalendarPage() {
  ...
  /** Same per-unit engine as Rooms & Inventory and the dashboard — see lib/admin/queries.ts. */
  const cellStatus = (unit: (typeof activeUnits)[number], day: string) =>
    unitStatusOn({ unit, bookings: bookings.data ?? [], blocks: blocks.data ?? [], date: day });
            ...
            <tbody>
              {activeUnits.map((u) => (
                <tr key={u.id} className="border-b border-hairline last:border-0">
                  ...
                  {days.map((d) => {
                    const { status, booking, block } = cellStatus(u, d);
                    return (
                      <td key={d} className="p-1 text-center align-middle">
                        {booking ? (
                          ...
                          <span
                            ...
                            className="block truncate rounded-md bg-orange-500/10 px-2 py-1.5 text-xs text-orange-700"
                          >
                            Maintenance
                          </span>
                        ) : status === "blocked" ? (
                          <span
                            title={block?.reason ?? undefined}
                            className="block truncate rounded-md bg-destructive/10 px-2 py-1.5 text-xs text-destructive"
                          >
                            Blocked
                          </span>
                        ) : (
